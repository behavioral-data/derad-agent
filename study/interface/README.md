...
# Already committed as study/data/notes.csv; re-run only if posts change.
python -m study.interface.extract_notes

# One-time media fetch (downloads attached tweet images/video stills via the X
# API into study/data/media/, writes media_index.csv). Both are committed, so this
...
- Bot replies are generated by `study/scripts/batch_generate_replies.py` and committed to
  `study/data/replies.csv`; `build_db` ingests them automatically. A post
