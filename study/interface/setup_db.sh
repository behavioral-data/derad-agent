#!/usr/bin/env bash
# Quick start: build the mock-X study database (study/data/study.db).
#
# Fast path (default): the committed data artifacts (posts.csv,
# notes.csv, media_index.csv + media/) are already in the repo,
...
# tsv_generation/cn_data_20260630/; fetch_media needs X API creds in agent/llm/.env.
#
...
if [ "$FULL" = "1" ]; then
  ...
  missing=0
  for f in study/data/posts.csv study/data/notes.csv study/data/media_index.csv; do
  ...
  if [ "$missing" = "1" ]; then
    ...
    echo "(extract_notes needs tsv_generation/cn_data_20260630/*.tsv; fetch_media needs X creds.)"
    exit 1
  fi
  STEP="1/1"
fi
...
"$PY" - <<'PYEOF'
import sqlite3
c = sqlite3.connect("study/data/study.db")
...
Note: bot replies are ingested from study/data/replies.csv
(committed). Any post without a generated reply falls back to stub text.
Regenerate replies with:
    python study/scripts/batch_generate_replies.py study/data/posts.csv -o study/data/replies.csv
EOF
